...
<div class="fieldcontain ${hasErrors(bean: roomInstance, field: 'lodgerType', 'error')} required">
	<td>
	<label for="lodgerType">
		<g:message code="room.lodgerType.label" default="Lodger Type" />
		<span class="required-indicator">*</span>
	</label>
	</td>
	
	<td><g:select name="lodgerType" from="${roomInstance.constraints.lodgerType.inList}" required="" value="${roomInstance?.lodgerType}" valueMessagePrefix="room.lodgerType"/></td>
</div>
</tr>
...
<div class="fieldcontain ${hasErrors(bean: roomInstance, field: 'vacancyStatus', 'error')} ">

	<g:hiddenField name="vacancyStatus" value="${vacancyStatus}"/>
</div>
